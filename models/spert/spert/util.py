...
def create_directories_dir(d):
    print('"create_directories_dir"', d)

    if d and not os.path.exists(d):
        os.makedirs(Path(rf"{d}"), exist_ok=True)
...
def check_version(config, model_class, model_path):
    if os.path.exists(model_path):
        ...
        if (
            "rel_classifier.weight" in state_dict
            and loaded_version != model_class.VERSION
        ):
            msg = (
                "Current SpERT version (%s) does not match the version of the loaded model (%s). "
